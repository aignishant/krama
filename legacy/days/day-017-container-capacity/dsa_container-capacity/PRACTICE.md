...
For nonnegative heights return max min(h[i],h[j])*(j-i), or 0 if fewer than two heights.
...
5. Run `python course.py practice 17` from the repository root. The starter is intentionally RED.
