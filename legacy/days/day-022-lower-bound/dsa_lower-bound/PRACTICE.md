...
Return first index with nums[i] >= target, or len(nums), for sorted nums.
...
5. Run `python course.py practice 22` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log n) time; O(1) space**.
