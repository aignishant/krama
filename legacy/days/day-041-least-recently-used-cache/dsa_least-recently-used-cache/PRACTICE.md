...
Process [put,key,value] and [get,key] operations with capacity >= 1. Return get results, using -1 for misses. Implement map plus doubly linked nodes.
...
5. Run `python course.py practice 41` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Expected O(1) per operation; O(capacity) space**.
