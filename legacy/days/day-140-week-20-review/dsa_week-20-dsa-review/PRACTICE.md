# Practice — Cold re-solve: Expiring counter
...
First re-solve [day 134](../../day-134-time-indexed-values/dsa_time-indexed-values/README.md), then the core below from
[day 138](../../day-138-expiring-counter/dsa_expiring-counter/README.md), both from blank code. A harder unresolved problem
...
Events are sorted nondecreasing integer timestamps; for each event return events in (t-window,t], including current and earlier equal-timestamp events; window > 0.
...
5. Run `python course.py practice 140` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) total time**.
