...
Return list of lowercase alphanumeric Unicode code points from text; use code points, not grapheme clusters.
...
5. Run `python course.py practice 120` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time plus output**.
