...
Directed nonnegative edges and multiple [src,dst] queries: return shortest distances, -1 for unreachable. Cache work per distinct source.
...
Input:
```json
{"n":3,"edges":[[0,1,2],[1,2,3]],"queries":[[0,2],[0,1],[2,0]]}
```
...
5. Run `python course.py practice 162` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Explain preprocessing versus per-query cost**.
