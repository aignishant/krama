# Practice — Cold re-solve: Longest distinct substring
...
First re-solve [day 15](../../day-015-sorted-pair-existence/dsa_sorted-pair-existence/README.md), then the core below from
[day 19](../../day-019-longest-distinct-substring/dsa_longest-distinct-substring/README.md), both from blank code. A harder unresolved problem
...
Return the length of the longest substring with no repeated Unicode code point.
...
5. Run `python course.py practice 21` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Expected O(n) time; O(k) space**.
