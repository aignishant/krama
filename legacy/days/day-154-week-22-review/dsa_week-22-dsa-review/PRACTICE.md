# Practice — Cold re-solve: Shortest subarray with negatives
...
First re-solve [day 148](../../day-148-interleaved-strings/dsa_interleaved-strings/README.md), then the core below from
[day 152](../../day-152-shortest-subarray-with-negatives/dsa_shortest-subarray-with-negatives/README.md), both from blank code. A harder unresolved problem
...
Return shortest nonempty subarray length with sum >= k, or -1; k > 0 and nums may be negative.
...
5. Run `python course.py practice 154` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time**.
