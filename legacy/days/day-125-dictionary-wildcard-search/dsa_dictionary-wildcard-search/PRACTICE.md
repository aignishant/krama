# Practice — Dictionary wildcard search
...
For each query return whether a lowercase dictionary word matches; dot matches one letter. Use a trie.
...
Input:
```json
{"words":["bad","dad","mad"],"queries":["pad","bad",".ad","b.."]}
```
...
5. Run `python course.py practice 125` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Query may branch exponentially in wildcard count**.
