...
Process [set,index,value] and [min,left,right] inclusive on nonempty nums; return minima. Use a segment tree.
...
Input:
```json
{"nums":[5,2,7,3],"ops":[["min",0,3],["set",1,8],["min",0,2]]}
```
...
5. Run `python course.py practice 118` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log n) per query/update**.
