# Practice — Cold re-solve: Strong components
...
First re-solve [day 78](../../day-078-nonnegative-shortest-paths/dsa_nonnegative-shortest-paths/README.md), then the core below from
[day 82](../../day-082-strong-components/dsa_strong-components/README.md), both from blank code. A harder unresolved problem
...
For directed graph return strongly connected vertex groups, each sorted, with groups lexicographically sorted.
...
5. Run `python course.py practice 84` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(V+E) traversal plus output sorting**.
