...
Return sorted unique triples summing to zero, each sorted internally. Input may be reordered.
...
5. Run `python course.py practice 16` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n^2) time excluding output**.
