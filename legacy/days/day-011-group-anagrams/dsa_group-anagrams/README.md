...
7. **Recall later without practice:** read [this day's recall card](../../../docs/DSA_RECALL.md#day-011-group-anagrams) for a previously studied topic. Reading is not learner completion.
...
**Budget:** 60 minutes. **Outcome:** DSA-11. **Theme:** Hashing and prefix sums.

## Prerequisite and recall

Prior DSA session: [day 10](../../day-010-pair-sum-indices/dsa_pair-sum-indices/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 10](../../day-010-pair-sum-indices/dsa_pair-sum-indices/README.md), [day 4](../../day-004-reverse-a-segment/dsa_reverse-a-segment/README.md)
...
| Understand | 10 | Read the core concepts, trace an example, check readiness, then compare the contract. |
...
**Group anagrams.** Group lowercase strings by letters. Sort strings within each group and sort the groups lexicographically.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: Explain character-key construction plus output sorting costs.
This is a target to justify, not a claim that any implementation meets it.

## Learn and explain

Read [CONCEPTS.md](CONCEPTS.md) before the first attempt; it explains the pattern, correctness, costs, and the online contract differences. The [pattern guide](../../../docs/DSA_PATTERN_GUIDE.md) is optional background.
