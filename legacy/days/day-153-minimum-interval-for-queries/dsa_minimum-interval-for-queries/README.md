# Day 153 DSA — Minimum interval for queries

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Minimum interval for queries** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-153. **Theme:** Mixed pattern selection II.

## Prerequisite and recall

Prior DSA session: [day 152](../../day-152-shortest-subarray-with-negatives/dsa_shortest-subarray-with-negatives/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 152](../../day-152-shortest-subarray-with-negatives/dsa_shortest-subarray-with-negatives/README.md), [day 146](../../day-146-find-duplicate-without-mutation/dsa_find-duplicate-without-mutation/README.md), [day 132](../../day-132-bipartite-matching/dsa_bipartite-matching/README.md)
...
**Minimum interval for queries.** For each integer query return smallest inclusive interval length containing it, or -1.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O((n+q) log(n+1)) time.
