# Practice — K largest stream values
...
Return the k largest values of nums sorted descending; 0 <= k <= n.
...
5. Run `python course.py practice 64` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n log(k+1)) plus output sorting**.
