...
Given unique [word,count] entries and prefix return at most k matches by descending count then lexicographic word; k >= 0.
...
Input:
```json
{"entries":[["cat",4],["car",4],["cart",2],["dog",9]],"prefix":"ca","k":2}
```
...
5. Run `python course.py practice 139` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Explain scan baseline and trie/top-k extension**.
