...
Merge k sorted arrays into one sorted list, retaining duplicates.
...
5. Run `python course.py practice 65` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(N log(k+1)) time**.
