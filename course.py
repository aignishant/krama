...
def validate(root=ROOT):
    ...
    for entry in entries:
        ...
        if directories != set(TRACKS):
            errors.append(f'Day {day}: expected exactly dsa, sd and lang folders; found {directories}.')
        required = ['LESSON.md', 'CHECKLIST.md', 'dsa/README.md', 'dsa/PRACTICE.md',
                    ...
                    'dsa/NOTES.md', 'sd/README.md', 'sd/DESIGN.md', 'lang/README.md',
        ...
        try:
            cases = json.loads((base / 'dsa/cases.json').read_text(encoding='utf-8'))
...
def practice(day, root=ROOT):
    ...
    test = root / 'days' / entry['folder'] / 'dsa/test_solution.py'
...
def main(argv=None):
    ...
    try:
        if args.command == 'start':
            ...
            path = ROOT / 'days' / entry['folder'] / args.track / 'README.md'
