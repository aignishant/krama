# Practice — Mock search

## Contract

Return minimum integer eating speed finishing positive piles within h hours; h >= number of piles and piles nonempty.
...
5. Run `python course.py practice 156` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n log(max(piles))) time**.
