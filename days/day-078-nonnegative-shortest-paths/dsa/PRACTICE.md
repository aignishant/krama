# Practice — Nonnegative shortest paths

## Contract

Directed weighted edges [u,v,w], w >= 0. Return distances from src; null for unreachable.
...
Input:
```json
{"n":4,"edges":[[0,1,4],[0,2,1],[2,1,2]],"src":0}
```
...
5. Run `python course.py practice 78` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O((V+E) log(V+1)) with heap**.
