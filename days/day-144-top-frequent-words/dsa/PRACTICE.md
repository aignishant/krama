# Practice — Top frequent words

## Contract

Return top k distinct words by descending count then lexicographic order; 0 <= k <= distinct count.
...
Input:
```json
{"words":["i","love","code","i","love","python"],"k":2}
```
...
5. Run `python course.py practice 144` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Count then O(u log u) baseline; heap extension**.
