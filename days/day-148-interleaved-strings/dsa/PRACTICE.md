# Practice — Interleaved strings

## Contract

Return whether c interleaves a and b while preserving each string's internal order.
...
5. Run `python course.py practice 148` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(len(a)*len(b)) time**.
