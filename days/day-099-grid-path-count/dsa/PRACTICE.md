# Practice — Grid path count

## Contract

Return number of top-left to bottom-right routes using right/down, avoiding 1-valued obstacles. Empty grid returns 0.
...
5. Run `python course.py practice 99` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(rows*cols) time; O(cols) state**.
