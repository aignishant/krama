# Practice — Queue from stacks

## Contract

Process [push,x] and [pop] operations; pops are valid. Return popped values. Use two stacks.
...
Input:
```json
{"ops":[["push",1],["push",2],["pop"],["push",3],["pop"]]}
```
...
5. Run `python course.py practice 44` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Amortized O(1) per operation**.
