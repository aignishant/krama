...
Return the numeric median of sorted a and b; total length is positive.
...
5. Run `python course.py practice 27` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log(min(n,m)+1)) time; stretch difficulty**.
