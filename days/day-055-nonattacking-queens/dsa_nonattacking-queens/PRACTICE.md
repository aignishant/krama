...
Return the number of ways to place n queens on n*n board with no shared row, column or diagonal; n >= 1.
...
5. Run `python course.py practice 55` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Exponential search with pruning**.
