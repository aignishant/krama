# Day 101 DSA — Common subsequence length

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Common subsequence length** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-101. **Theme:** Dynamic programming on grids and strings.

## Prerequisite and recall

Prior DSA session: [day 100](../../day-100-minimum-grid-path/dsa_minimum-grid-path/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 100](../../day-100-minimum-grid-path/dsa_minimum-grid-path/README.md), [day 94](../../day-094-minimum-coins/dsa_minimum-coins/README.md), [day 80](../../day-080-disjoint-set-queries/dsa_disjoint-set-queries/README.md)
...
**Common subsequence length.** Return longest common subsequence length of strings a and b.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(nm) time; O(min(n,m)) optional space.
