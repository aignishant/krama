# Practice — Common subsequence length
...
Return longest common subsequence length of strings a and b.
...
5. Run `python course.py practice 101` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(nm) time; O(min(n,m)) optional space**.
