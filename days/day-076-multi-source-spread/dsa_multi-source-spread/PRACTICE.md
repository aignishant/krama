...
Grid 0 is empty, 1 fresh, 2 active. Active cells spread orthogonally each step. Return steps to activate all fresh cells or -1.
...
5. Run `python course.py practice 76` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(rows*cols) time**.
