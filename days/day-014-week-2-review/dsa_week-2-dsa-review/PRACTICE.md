...
First re-solve [day 8](../../day-008-first-repeated-value/dsa_first-repeated-value/README.md), then the core below from
[day 12](../../day-012-range-sums/dsa_range-sums/README.md), both from blank code. A harder unresolved problem
...
For each inclusive [left,right] query return the sum. Queries are valid for nums; an empty nums has no queries.
...
5. Run `python course.py practice 14` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n+q) time; O(n) space**.
