# Practice — Find duplicate without mutation
...
Length n+1 nums contains values 1..n and exactly one distinct duplicated value, possibly repeated multiple times. Return it without modifying nums.
...
5. Run `python course.py practice 146` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) space**.
