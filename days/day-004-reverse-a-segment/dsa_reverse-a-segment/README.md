...
7. **Recall later without practice:** read the [DSA summary file](../../../docs/DSA_RECALL.md#day-004-reverse-a-segment) for previously studied topics. Cards appear as days are taught; their presence does not mark study complete.
...
**Budget:** 60 minutes. **Outcome:** DSA-04. **Theme:** Cost, invariants and arrays.

## Prerequisite and recall

Prior DSA session: [day 3](../../day-003-stable-compaction/dsa_stable-compaction/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 3](../../day-003-stable-compaction/dsa_stable-compaction/README.md)
...
**Reverse a segment.** Reverse nums between inclusive indices left and right; return the modified list. Inputs satisfy 0 <= left <= right < n.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(right-left+1) time; O(1) space.
...
- [ ] Explain mirrored pairs, the inclusive boundary, termination, and untouched positions.
