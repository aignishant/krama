# Practice — Cold re-solve: Two-color graph
...
First re-solve [day 71](../../day-071-reachability/dsa_reachability/README.md), then the core below from
[day 75](../../day-075-two-color-graph/dsa_two-color-graph/README.md), both from blank code. A harder unresolved problem
...
Return whether an undirected graph is bipartite; include disconnected components.
...
5. Run `python course.py practice 77` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(V+E) time**.
