...
Return the lexicographically smallest index pair [i,j] with i < j and nums[i]+nums[j]=target; return [] if absent.
...
5. Run `python course.py practice 10` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Expected O(n) time; O(n) space**.
