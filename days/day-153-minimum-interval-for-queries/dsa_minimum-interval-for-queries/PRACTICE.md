# Practice — Minimum interval for queries
...
For each integer query return smallest inclusive interval length containing it, or -1.
...
5. Run `python course.py practice 153` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O((n+q) log(n+1)) time**.
