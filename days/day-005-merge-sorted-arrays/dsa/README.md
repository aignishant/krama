...
Prior DSA session: [day 4](../../day-004-reverse-a-segment/dsa/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 4](../../day-004-reverse-a-segment/dsa/README.md)
...
[HINTS.md](HINTS.md), so the first attempt remains independent. Expected target: O(n+m) time; output space O(n+m).
