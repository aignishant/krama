# Day 079 DSA — Negative edge paths

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Negative edge paths** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-79. **Theme:** Weighted graphs and connectivity.

## Prerequisite and recall

Prior DSA session: [day 78](../../day-078-nonnegative-shortest-paths/dsa_nonnegative-shortest-paths/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 78](../../day-078-nonnegative-shortest-paths/dsa_nonnegative-shortest-paths/README.md), [day 72](../../day-072-unweighted-distance/dsa_unweighted-distance/README.md), [day 58](../../day-058-level-order/dsa_level-order/README.md)
...
**Negative edge paths.** Return distances from src in directed weighted graph; return string negative-cycle if a negative cycle is reachable from src.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(VE) time.
