# Practice — Negative edge paths

## Contract

Return distances from src in directed weighted graph; return string negative-cycle if a negative cycle is reachable from src.
...
5. Run `python course.py practice 79` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(VE) time**.
