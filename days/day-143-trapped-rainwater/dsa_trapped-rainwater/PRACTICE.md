...
Return total trapped water above nonnegative unit-width bars.
...
5. Run `python course.py practice 143` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) auxiliary stretch**.
