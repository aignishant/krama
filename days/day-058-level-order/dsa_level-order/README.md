# Day 058 DSA — Level order

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Level order** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-58. **Theme:** Binary trees and search trees.

## Prerequisite and recall

Prior DSA session: [day 57](../../day-057-tree-depth/dsa_tree-depth/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 57](../../day-057-tree-depth/dsa_tree-depth/README.md), [day 51](../../day-051-enumerate-permutations/dsa_enumerate-permutations/README.md), [day 37](../../day-037-middle-node/dsa_middle-node/README.md)
...
**Level order.** Return one list of values per depth, left to right, for nested-list trees.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(n) time; O(width) queue space.
