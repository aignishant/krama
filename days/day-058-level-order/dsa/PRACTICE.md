...
Return one list of values per depth, left to right, for nested-list trees.
...
5. Run `python course.py practice 58` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(width) queue space**.
