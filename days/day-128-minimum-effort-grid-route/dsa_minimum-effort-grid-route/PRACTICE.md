# Practice — Minimum effort grid route
...
Return minimum possible maximum absolute height difference on a top-left to bottom-right orthogonal path; nonempty grid.
...
5. Run `python course.py practice 128` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(RC log(RC)) time**.
