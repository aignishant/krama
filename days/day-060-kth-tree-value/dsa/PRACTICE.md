# Practice — Kth tree value

## Contract

For a strict binary search tree return kth smallest value; k is valid and one-based.
...
Input:
```json
{"tree":[3,[1,null,[2,null,null]],[4,null,null]],"k":2}
```
...
5. Run `python course.py practice 60` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(height+k) time with iterative traversal**.
