...
Return product of all other values for each position; no division; empty returns [].
...
5. Run `python course.py practice 141` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) auxiliary excluding output**.
