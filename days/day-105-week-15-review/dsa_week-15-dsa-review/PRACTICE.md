# Practice — Cold re-solve: Longest palindrome length
...
First re-solve [day 99](../../day-099-grid-path-count/dsa_grid-path-count/README.md), then the core below from
[day 103](../../day-103-longest-palindrome-length/dsa_longest-palindrome-length/README.md), both from blank code. A harder unresolved problem
...
Return length of longest contiguous palindromic substring; empty returns 0. Implement interval DP.
...
5. Run `python course.py practice 105` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n^2) time**.
