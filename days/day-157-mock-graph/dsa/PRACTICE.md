...
Return number of shortest unweighted paths from src to dst in a simple undirected graph; src==dst has one empty path.
...
Input:
```json
{"n":4,"edges":[[0,1],[0,2],[1,3],[2,3]],"src":0,"dst":3}
```
...
5. Run `python course.py practice 157` from the repository root. The starter is intentionally RED.
