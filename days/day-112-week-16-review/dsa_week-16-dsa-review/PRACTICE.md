# Practice — Cold re-solve: Matrix chain cost
...
First re-solve [day 106](../../day-106-zero-one-knapsack/dsa_zero-one-knapsack/README.md), then the core below from
[day 110](../../day-110-matrix-chain-cost/dsa_matrix-chain-cost/README.md), both from blank code. A harder unresolved problem
...
dims describes compatible matrices dims[i]*dims[i+1]. Return minimum scalar multiplications; at least two positive dims.
...
5. Run `python course.py practice 112` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n^3) time; O(n^2) space**.
