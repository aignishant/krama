# Day 129 DSA — Redundant undirected edge

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Redundant undirected edge** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-129. **Theme:** Advanced graph and tree practice.

## Prerequisite and recall

Prior DSA session: [day 128](../../day-128-minimum-effort-grid-route/dsa_minimum-effort-grid-route/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 128](../../day-128-minimum-effort-grid-route/dsa_minimum-effort-grid-route/README.md), [day 122](../../day-122-find-all-pattern-matches/dsa_find-all-pattern-matches/README.md), [day 108](../../day-108-signed-target-count/dsa_signed-target-count/README.md)
...
**Redundant undirected edge.** A tree has one extra edge added. Return the first input edge whose insertion connects already-connected vertices.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: Near-linear with disjoint sets.
