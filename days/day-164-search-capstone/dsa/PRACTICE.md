# Practice — Search capstone

## Contract

Given unique words, return all dictionary words present on an orthogonal grid path without cell reuse, sorted; words are nonempty.
...
Input:
```json
{"board":[["a","b"],["c","d"]],"words":["ab","abd","ac","aba"]}
```
...
5. Run `python course.py practice 164` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **Trie-guided exponential search; state assumptions**.
