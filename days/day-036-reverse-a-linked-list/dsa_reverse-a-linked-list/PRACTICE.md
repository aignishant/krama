...
Build singly linked nodes from values, reverse links, serialize the resulting values. Do not solve by reversing the input list.
...
5. Run `python course.py practice 36` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) reversal workspace, excluding adapter**.
