...
Return maximum sum from a nonempty circular array of nonnegative houses, with no adjacent selected houses; one house may be selected.
...
5. Run `python course.py practice 158` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) auxiliary**.
