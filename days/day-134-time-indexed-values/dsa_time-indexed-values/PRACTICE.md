...
Process [set,key,value,timestamp] with strictly increasing timestamps per key, and [get,key,timestamp]. Return latest value <= timestamp or null.
...
Input:
```json
{"ops":[["set","a","x",1],["set","a","y",3],["get","a",2],["get","a",0]]}
```
...
5. Run `python course.py practice 134` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log n) get; amortized O(1) append**.
