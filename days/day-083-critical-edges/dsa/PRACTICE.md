# Practice — Critical edges

## Contract

In a simple undirected graph return sorted bridges [min(u,v),max(u,v)], lexicographically sorted.
...
5. Run `python course.py practice 83` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(V+E) traversal plus sorting; stretch**.
