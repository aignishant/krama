# Practice — Find all pattern matches

## Contract

Return all starting indices where pattern occurs in text, including overlaps; empty pattern matches 0..len(text). Use prefix-function matching.
...
5. Run `python course.py practice 122` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n+m) time**.
