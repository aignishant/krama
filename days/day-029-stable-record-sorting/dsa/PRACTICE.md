# Practice — Stable record sorting

## Contract

Return records sorted ascending by score while preserving original order on ties. Implement merge sort once.
...
5. Run `python course.py practice 29` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n log n) time; O(n) space**.
