# Practice — Cold re-solve: Word search
...
First re-solve [day 50](../../day-050-enumerate-subsets/dsa_enumerate-subsets/README.md), then the core below from
[day 54](../../day-054-word-search/dsa_word-search/README.md), both from blank code. A harder unresolved problem
...
Return whether word can be formed by orthogonal steps in a rectangular character grid without reusing a cell. Empty word is true.
...
5. Run `python course.py practice 56` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(rows*cols*4^L) upper bound**.
