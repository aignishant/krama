# Practice — Prefix dictionary

## Contract

Build a trie of words. For each prefix return how many inserted distinct words begin with it; empty prefix matches all.
...
Input:
```json
{"words":["car","cat","dog","car"],"prefixes":["ca","c","z",""]}
```
...
5. Run `python course.py practice 67` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(total characters + query characters)**.
