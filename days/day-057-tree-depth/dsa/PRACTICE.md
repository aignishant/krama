...
Trees use null or [value,left,right]. Return maximum number of nodes on a root-to-leaf path.
...
Input:
```json
{"tree":[1,[2,null,null],[3,[4,null,null],null]]}
```
...
5. Run `python course.py practice 57` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(height) call stack**.
