# Practice — Sliding window maxima

## Contract

Return maximum of each length-k window; 1 <= k <= n.
...
5. Run `python course.py practice 48` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(k) space**.
