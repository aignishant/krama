# Practice — Equal partition

## Contract

Return whether nonnegative nums can be split into two equal-sum subsets; empty is true.
...
5. Run `python course.py practice 107` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n*sum(nums)) time upper bound**.
