# Day 107 DSA — Equal partition

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Equal partition** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-107. **Theme:** Knapsack and advanced DP states.

## Prerequisite and recall

Prior DSA session: [day 106](../../day-106-zero-one-knapsack/dsa_zero-one-knapsack/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 106](../../day-106-zero-one-knapsack/dsa_zero-one-knapsack/README.md), [day 100](../../day-100-minimum-grid-path/dsa_minimum-grid-path/README.md), [day 86](../../day-086-reach-the-end/dsa_reach-the-end/README.md)
...
**Equal partition.** Return whether nonnegative nums can be split into two equal-sum subsets; empty is true.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(n*sum(nums)) time upper bound.
