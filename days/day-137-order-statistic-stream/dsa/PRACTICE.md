# Practice — Order statistic stream

## Contract

After each insertion return the kth largest so far, or null until k values exist; k >= 1.
...
5. Run `python course.py practice 137` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log k) per insertion**.
