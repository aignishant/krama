# Practice — Cold re-solve: Remove from end
...
First re-solve [day 36](../../day-036-reverse-a-linked-list/dsa_reverse-a-linked-list/README.md), then the core below from
[day 40](../../day-040-remove-from-end/dsa_remove-from-end/README.md), both from blank code. A harder unresolved problem
...
Build a linked list and remove its nth node from the end; 1 <= n <= length. Return serialized values.
...
5. Run `python course.py practice 42` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(n) time; O(1) traversal space**.
