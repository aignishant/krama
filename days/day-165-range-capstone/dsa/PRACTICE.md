# Practice — Range capstone

## Contract

Process inclusive range queries [left,right] returning number of distinct values in nums; indices valid.
...
Input:
```json
{"nums":[1,2,1,3],"queries":[[0,2],[1,3],[2,2]]}
```
...
5. Run `python course.py practice 165` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O((n+q) log n) offline target; stretch**.
