# Practice — Modular power

## Contract

Return (base**exp) modulo mod, with exp >= 0 and mod >= 1; do not call three-argument pow.
...
5. Run `python course.py practice 115` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log(exp+1)) multiplications**.
