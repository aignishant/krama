# Day 115 DSA — Modular power

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Modular power** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-115. **Theme:** Bits, arithmetic and range structures.

## Prerequisite and recall

Prior DSA session: [day 114](../../day-114-set-bit-counts/dsa_set-bit-counts/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 114](../../day-114-set-bit-counts/dsa_set-bit-counts/README.md), [day 108](../../day-108-signed-target-count/dsa_signed-target-count/README.md), [day 94](../../day-094-minimum-coins/dsa_minimum-coins/README.md)
...
**Modular power.** Return (base**exp) modulo mod, with exp >= 0 and mod >= 1; do not call three-argument pow.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(log(exp+1)) multiplications.
