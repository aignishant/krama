# Day 038 DSA — Cycle entry

## Navigation — where to start

1. **Prepare before practice:** read the [DSA pattern guide](../../../docs/DSA_PATTERN_GUIDE.md). This is a shared preparation guide; the full explanation of **Cycle entry** is not yet expanded.
...
**Budget:** 60 minutes. **Outcome:** DSA-38. **Theme:** Linked lists and pointer invariants.

## Prerequisite and recall

Prior DSA session: [day 37](../../day-037-middle-node/dsa_middle-node/README.md). Read only its notes if already comfortable.
Recall one candidate, not all of them: [day 37](../../day-037-middle-node/dsa_middle-node/README.md), [day 31](../../day-031-insert-an-interval/dsa_insert-an-interval/README.md), [day 17](../../day-017-container-capacity/dsa_container-capacity/README.md)
...
**Cycle entry.** Build nodes from values and link the tail to index pos, or no cycle for -1. Return entry index or -1. Nonempty when pos >= 0.

After learning the topic, open [PRACTICE.md](PRACTICE.md). General mechanisms belong in
the explanation; [HINTS.md](HINTS.md) helps apply them after an attempt. Expected target: O(n) time; O(1) detection space.
