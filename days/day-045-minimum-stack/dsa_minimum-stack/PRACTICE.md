...
Process [push,x], [pop], and [min]; pop and min are valid. Return min results.
...
Input:
```json
{"ops":[["push",3],["push",1],["push",1],["pop"],["min"]]}
```
...
5. Run `python course.py practice 45` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(1) per operation**.
