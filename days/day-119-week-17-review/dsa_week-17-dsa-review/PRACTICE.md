# Practice — Cold re-solve: Mutable range sums
...
First re-solve [day 113](../../day-113-single-unmatched-value/dsa_single-unmatched-value/README.md), then the core below from
[day 117](../../day-117-mutable-range-sums/dsa_mutable-range-sums/README.md), both from blank code. A harder unresolved problem
...
Process [add,index,delta] and [sum,left,right] inclusive over nums; valid indices. Return sums. Use a binary indexed tree.
...
Input:
```json
{"nums":[1,2,3],"ops":[["sum",0,2],["add",1,4],["sum",1,2]]}
```
...
5. Run `python course.py practice 119` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log n) per operation**.
