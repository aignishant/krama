...
Return floor(sqrt(n)) for nonnegative integer n, without square-root functions.
...
5. Run `python course.py practice 25` from the repository root. The starter is intentionally RED.
...
9. Explain correctness, time, auxiliary space and output space. Target: **O(log n) comparisons**.
